function commentTimeHandle(dateStr) {
  ...
  var publishTime = dateStr / 1000,  //获取dataStr的秒数  打印结果--1536230820000
    ...
    var diffValue = nowTime - publishTime;  // 获取此时 秒数 与 要处理的日期秒数 之间的差值
    let diff_days = parseInt(diffValue / 86400)    // 一天86400秒 获取相差的天数 取整
    let diff_hours = parseInt(diffValue / 3600)    // 一时3600秒
    let diff_minutes = parseInt(diffValue / 60)
    let diff_secodes = parseInt(diffValue)
    if (diff_secodes <= 0) {
        return "刚刚";
    }else if(diff_secodes < 60) {
        return diff_secodes + "秒前";
    }else if (diff_minutes > 0&&diff_hours<=0) {
        return diff_minutes + "分钟前";
    }else if (diff_days <= 0 && diff_hours > 0) {
        return diff_hours + "小时前";
    }else if (diff_days > 0 && diff_days < 3) {  //相差天数 0 < diff_days < 3 时, 直接返出
        return diff_days + "天前";
